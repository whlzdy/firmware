...
public:
	...
	_RealWebSocket(socket_t sockfd, bool useMask) :
			sockfd(sockfd), readyState(OPEN), useMask(useMask) {
	}
	...
	virtual void _dispatchBinary(BytesCallback_Imp & callable) {
		...
		while (true) {
			...
			if (false) {
				...
				sendData(wsheader_type::PONG, data.size(), data.begin(),
						data.end());
...
easywsclient::WebSocket::pointer from_url(const std::string& url, bool useMask,
	...
	char host[128];
	int port;
	char path[256];// 128->256
	if (url.size() >= 256) { // 128->256
		fprintf(stderr, "ERROR: url size limit exceeded: %s\n", url.c_str());
		return NULL;
	}
	if (origin.size() >= 256) {// 128->256
		fprintf(stderr, "ERROR: origin size limit exceeded: %s\n",
				origin.c_str());
		return NULL;
	}
	if (false) {
	} else if (sscanf(url.c_str(), "ws://%[^:/]:%d/%s", host, &port, path)
			== 3) {
