...
static void* thread_com_temperature_handle(void*);
...
int main_busi_unsupport_command(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
int main_busi_ctrl_temperature(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
int main_busi_query_temperature(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
...
int load_config(char *config) {
	...
	g_config->role = role_temperature;

	temp_int = 0;
	get_parameter_int(SECTION_TEMPERATURE, "debug", &temp_int, OC_DEBUG_LEVEL_DISABLE);
...
int main(int argc, char **argv) {
	...
	if (argc > 1) {
		if (strcmp("help", argv[1]) == 0 || strcmp("-h", argv[1]) == 0
			...
			fprintf(stderr, "usage: temperature_daemon [configuration file]\n");
			exit(1);
		}
	...
	s_addr_in.sin_port = htons(g_config->temperature_port);
	...
	log_debug_print(g_debug_verbose, "com temperature thread!");
	pthread_t thread_id_temperature;
	if (pthread_create(&thread_id_temperature, NULL, thread_com_temperature_handle,
	NULL) == -1) {
...
int main_business_process(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
	...
	switch (command) {
	case OC_REQ_QUERY_TEMPERATURE:
		result = main_busi_query_temperature(req_buf, req_len, resp_buf, resp_len);
		break;
...
int main_busi_query_temperature(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
	...
	OC_CMD_QUERY_TEMPERATURE_REQ* request = (OC_CMD_QUERY_TEMPERATURE_REQ*) (req_buf
			+ OC_PKG_FRONT_PART_SIZE);
	OC_CMD_QUERY_TEMPERATURE_RESP* response = NULL;
	uint8_t* busi_buf = NULL;
	uint8_t* temp_buf = NULL;
	int busi_buf_len = 0;
	int temp_buf_len = 0;
	result = generate_cmd_query_temperature_resp(&response, exec_result, error_no, t1, t2);
	result = translate_cmd2buf_query_temperature_resp(response, &busi_buf, &busi_buf_len);
	if (result == OC_SUCCESS) {
		result = generate_response_package(OC_REQ_QUERY_TEMPERATURE, busi_buf, busi_buf_len,
...
static void* thread_com_temperature_handle(void*) {
	...
	while (1) {
		...
		script_shell_exec((unsigned char*) "/opt/onecloud/script/temperature/getusbno.sh",
				script_result);
		if (strlen((char*) script_result) < 2) {
			log_error_print(g_debug_verbose, "temperature device not found! retry in 60s.");
			sleep(60);
			continue;
		}
		...
		if (init_serial((char*) device_name) != 0) {
			log_error_print(g_debug_verbose,
					"init_serial temperature device error %s. retry in 60s.", device_name);
			sleep(60);
			continue;
		}
		...
		while (1) {
			...
			if (nRecv > 1) {
				if (memcmp(buf_recv, "t1", 2) == 0) {
					t1 = atof(buf_recv + 3);
				}
				if (memcmp(buf_recv, "t2", 2) == 0) {
					t2 = atof(buf_recv + 3);
				}
			} else {
				t1 = 0;
...
void timer_event_watchdog_heartbeat() {
	...
	uint32_t device = device_temperature;
