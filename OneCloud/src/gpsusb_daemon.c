...
static void* thread_com_gpsusb_handle(void*);
// function definition
...
int main_busi_unsupport_command(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
int main_busi_ctrl_gpsusb(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
int main_busi_query_gpsusb(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
		int* resp_len);
...
int load_config(char *config) {
	...
	g_config->role = role_gpsusb;

	temp_int = 0;
	get_parameter_int(SECTION_GPSUSB, "debug", &temp_int, OC_DEBUG_LEVEL_DISABLE);
	g_debug_verbose = temp_int;

	temp_int = 0;
	get_parameter_int(SECTION_GPSUSB, "enable", &temp_int, 0);
	if(temp_int == OC_FALSE){
		// disabled
		log_info_print(g_debug_verbose, "gpsusb_daemon is disabled, exit");
		exit(EXIT_SUCCESS);
	}
...
int main(int argc, char **argv) {
	...
	if (argc > 1) {
		if (strcmp("help", argv[1]) == 0 || strcmp("-h", argv[1]) == 0
			...
			fprintf(stderr, "usage: gpsusb_daemon [configuration file]\n");
			exit(1);
		}
	...
	s_addr_in.sin_port = htons(g_config->gpsusb_port);
	...
	log_debug_print(g_debug_verbose, "com gpsusb thread!");
	pthread_t thread_id_gpsusb;
	if (pthread_create(&thread_id_gpsusb, NULL, thread_com_gpsusb_handle,
	NULL) == -1) {
		log_error_print(g_debug_verbose, "com gpsusb pthread_create error!");
		return EXIT_FAILURE;
	}
...
int main_business_process(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
	...
	switch (command) {
	case OC_REQ_QUERY_GPSUSB:
		result = main_busi_query_gpsusb(req_buf, req_len, resp_buf, resp_len);
		break;
...
int main_busi_query_gpsusb(unsigned char* req_buf, int req_len, unsigned char* resp_buf,
	...
	OC_CMD_QUERY_GPSUSB_REQ* request = (OC_CMD_QUERY_GPSUSB_REQ*) (req_buf
			+ OC_PKG_FRONT_PART_SIZE);
	OC_CMD_QUERY_GPSUSB_RESP* response = NULL;
	uint8_t* busi_buf = NULL;
	uint8_t* temp_buf = NULL;
	int busi_buf_len = 0;
	int temp_buf_len = 0;
	result = generate_cmd_query_gpsusb_resp(&response, exec_result, error_no, ew,longitude,ns,latitude,altitude);
	result = translate_cmd2buf_query_gpsusb_resp(response, &busi_buf, &busi_buf_len);
	if (result == OC_SUCCESS) {
		result = generate_response_package(OC_REQ_QUERY_GPSUSB, busi_buf, busi_buf_len,
...
static void* thread_com_gpsusb_handle(void*) {
	...
	while (1) {
		...
		script_shell_exec((unsigned char*) "/opt/onecloud/script/gpsusb/getusbno.sh",
				script_result);
		log_debug_print(g_debug_verbose, "script_result [%s]", script_result);
		if (strlen((char*) script_result) < 2) {
			log_error_print(g_debug_verbose, "gpsusb control device not found!");
			exit(EXIT_FAILURE);
		}
		...
		if (init_serial((char*) device_name) != 0) {
			log_error_print(g_debug_verbose,
					"init_serial gpsusb device error %s. retry in 60s.", device_name);
			sleep(60);
			continue;
		}
		...
		while (1) {
			...
			if(nRecv<=0){
				log_debug_print(g_debug_verbose, "gpsusb data device_name %s no data", device_name);
				break;
			}
			...
			if(flag==1){
				...
				//		valid, tmp_ew, tmp_longitude, tmp_ns, tmp_latitude, tmp_altitude);
				if(valid==1){
					ew=tmp_ew;
